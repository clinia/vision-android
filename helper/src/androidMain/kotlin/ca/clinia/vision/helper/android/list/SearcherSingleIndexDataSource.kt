...
public class SearcherSingleIndexDataSource<T>(
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, T>) {
        ...
        runBlocking {
            try {
                ...
                val nextKey = if (response.meta.total > initialLoadSize) 1 else null

                withContext(searcher.coroutineScope.coroutineContext) {
                    searcher.response.value = response
                    searcher.isLoading.value = false
                }
                callback.onResult(response.records.map(transformer), 0, response.meta.total, null, nextKey)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, T>) {
        ...
        runBlocking {
            try {
                val response = searcher.search()
                val nextKey = if (page + 1 < response.meta.total) params.key + 1 else null

                withContext(searcher.coroutineScope.coroutineContext) {
                    searcher.response.value = response
                    searcher.isLoading.value = false
                }
                callback.onResult(response.records.map(transformer), nextKey)
            } catch (throwable: Throwable) {
                withContext(searcher.coroutineScope.coroutineContext) {
                    searcher.error.value = throwable
                    searcher.isLoading.value = false
